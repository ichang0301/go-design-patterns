...
func TestPizzaDecorator_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When calling the add ingredient of the pizza decorator, it must return the expected text '%s'. but got '%s'", expectedText, pizzaResult)
	}
}
...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the onion decorator without an IngredientAdd on its Ingredient field, it must return an error, not a string with '%s'", onionResult)
	}
...
func TestMeat_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the meat decorator without an IngredientAdd on its Ingredient field, it must return an error, not a string with '%s'", meatResult)
	}
